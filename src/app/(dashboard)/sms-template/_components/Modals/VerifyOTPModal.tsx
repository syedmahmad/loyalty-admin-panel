...
const OtpModal: React.FC<OtpModalProps> = ({ open, onClose, templateId, setOpenCreateSender, reFetch }) => {
  ...
  const handleVerify = async () => {
    ...
    try {
      const response = await POST("templates/verify-otp", { templateId, otp: enteredOtp }, {
        headers: {
      ...
      if (response?.status === 201) {
        toast.success('OTP verified successfully!, your template will be active after internal verification');
        reFetch();
        setStep('send');
        setOtp(['', '', '', '']);
        onClose();
