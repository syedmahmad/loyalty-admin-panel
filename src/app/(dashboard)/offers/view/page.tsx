...
const OfferList = () => {
  ...
  const handleCloseDrawer = () => {
    router.push("/offers/view");
  };

  const [selectedOffer, setSelectedOffer] = useState<null | Offer>(null);
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    const res = await DELETE(`/offers/${deleteId}`);
    if (res?.status === 200) {
      toast.success("Offer deleted!");
      fetchOffers(search, selectedBU, pageNumber, pageSize);
    } else {
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setSearch(value);
    setTimeout(() => {
      fetchOffers(value, selectedBU, pageNumber, pageSize);
    }, 300);
  };

  useEffect(() => {
    if (selectedBU !== undefined && selectedBU !== null && selectedBU !== 0) {
      fetchOffers(search, selectedBU, pageNumber, pageSize);
    }
  }, [selectedBU]);
  ...
  const paginationOffer = offers.slice(
    page * rowsPerPage,
    ...
    <Box sx={{ backgroundColor: "#F9FAFB", mt: "-25px" }}>
      ...
      >
        ...
        <Box sx={{ gap: 1, display: "flex" }}>
          ...
          <Button
            variant="outlined"
            onClick={() => router.push("/offers/view?drawer=create")}
            sx={{
              backgroundColor: "#fff",
              fontFamily: "Outfit",
              fontWeight: 600,
      ...
      >
          ...
          <Box component={Paper}>
                ...
                <TableBody>
                  {offers.map((offer: any) => (
                    <TableRow key={offer.uuid}>
                      ...
                      >
                        <Tooltip title={offer.business_unit?.name || "-"}>
            ...
            >
              ...
              <Pagination
                ...
                onChange={(_, value) => {
                  setPageNumber(value);
                  fetchOffers(search, selectedBU, value, pageSize);
                }}
                shape="rounded"
                siblingCount={1}
                boundaryCount={1}
        ...
        <BaseDrawer
          ...
          title={drawerType == "bulkupload" ? "Upload Offers" : "Create Offer"}
          width={drawerWidth}
