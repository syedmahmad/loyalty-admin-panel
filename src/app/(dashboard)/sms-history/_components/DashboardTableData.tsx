  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }: any) => {
        ...
        } else if (row.original.isSend === true && row.original.response_code !== "000" && row.original.response_code !== null) {
          statusToShow = "Failed";
        }
    ...
    {
      accessorKey: "sms_gateway",
      header: "Gateway",
      cell: ({ row }: any) => <>{row.original.sms_gateway ?? "N/A"}</>,
    },
    {
