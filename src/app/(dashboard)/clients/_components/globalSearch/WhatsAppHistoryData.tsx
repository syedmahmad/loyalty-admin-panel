...
export const WhatsAppTableData = ({
  ...
  const columns = [
    ...
    },
    {
      ...
      header: "Reason",
      cell: ({ row }: any) => {
        const un_snd_reason: string = !row.original.unsend_reason
          ? "N/A"
          : row.original.unsend_reason
        const isTruncated = un_snd_reason?.length > 20;
        const displayValue = isTruncated
          ? un_snd_reason?.slice(0, 20) + '...'
          : un_snd_reason;
        
        return (
          <Tooltip title={isTruncated ? un_snd_reason : ''} arrow>
            <span>{displayValue}</span>
          </Tooltip>
        );
      },
      ...
      <Grid2 container spacing={1} columns={15}>
        ...
        {/* <Grid2 xs={7.5} md={3}>
          ...
          >
            ...
            <MenuItem value="000">Successful</MenuItem>
