...
import { Card, CardContent, Typography, IconButton, Box, Stack, Button } from "@mui/material";
...
export const SingleWhatsappSender = ({
  sender,
  handleEditSender,
  handleDeleteSender,
  reFetch,
      ...
      <Card sx={{ position: "relative", p: 2 }}>
        ...
        <CardContent>
          ...
          >
            ...
            {sender?.locales[0]?.country_code} {sender?.locales[0]?.sender_phone_no}
          </Typography>
          {/* 
          <Typography
            variant="body2"
          ...
          {/* {!sender.active ? <ActionButtons refetch={reFetch} sender={sender} /> : <PlaceholderBox />} */}
        </CardContent>
      </Card>
    </Grid2>
  );
...
function ActionButtons({ refetch, sender }: any) {
  ...
  const handleReSendOtp = async () => {
    ...
    try {
      const res = await POST("senders/send-otp", { senderId: sender.id, user_email: userDetails.email! });
      ...
      toast.error("Verification failed. Please try again. Or check your selected sender email");
      setLoading(false);
    }
  }
  return (
