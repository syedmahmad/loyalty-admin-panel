...
import { EditSenderModal } from "./Modals/EditSenderModal";
import { GET } from "@/utils/AxiosUtility";
import TableLoader from "@/components/loaders/TableLoader";
import NotFound from "@/components/errors/404";
import { useDebounce } from "use-debounce";
import { DeleteSenderModal } from "./Modals/DeleteSenderModal";
import CreateWhatsappSenderModal from "./CreateWhatsappSenderModal";
import { SingleWhatsappSender } from "./SingleWhatsappSender";

export const WhatsappSenderQuery = () => {
  ...
  const reFetch = () => {
    ...
    queryClient.refetchQueries({ queryKey: ["get-all-whatsapp-senders"] });
  };

  const getAllSendersQuery = useQuery({
    queryKey: ["get-all-whatsapp-senders"],
    queryFn: async () => {
      const endpoint =
        search.trim() !== "" && search !== ""
          ? `senders/search?query=${encodeURIComponent(
            search
          )}&clientId=${clientId}&type=whatsapp`
          : `senders/all-sender/whatsapp?clientId=${clientId}`;
      ...
      <Grid2 container>
        <Grid2 xs={12}>
          ...
          >
            ...
            >
              Whatsapp Senders
              <Tooltip
                title="You can view a list of whatsapp senders here."
                arrow
                placement="top"
              >
                <InfoOutlinedIcon
        ...
        <Grid2>
          <Alert style={{ fontSize: "14px" }}>
            ...
            in our whatsapp Gateways. Otherwise, the sender will not function
            correctly. <br /><strong>The sender will be created immediately but once you create a template using this sender. We will
            send a verification message to the sender&apos;s phone number to confirm that the sender in registered in our Whatsapp Gateway.</strong>
          </Alert>
          <br />
        </Grid2>
      </Grid2>
