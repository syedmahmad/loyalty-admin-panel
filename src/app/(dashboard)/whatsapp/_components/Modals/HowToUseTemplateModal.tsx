...
import { Box, Dialog, IconButton, Typography, Paper } from "@mui/material";
...
export const HowToUseWhatsAppTemplateModal = ({
  ...
  const [dynamicFieldsForHeader, setDynamicFieldsForHeader] = useState<{
    [key: string]: string;
  }>({});

  const [dynamicFieldsForFooter, setDynamicFieldsForFooter] = useState<{
    [key: string]: string;
  }>({});
  ...
  useEffect(() => {
    ...
    if (templateToBeUsed?.translations?.[0]?.header) {
      const headerFields = extractDynamicFields(
        templateToBeUsed?.translations?.[0]?.header
      );
      const initialHeaderFields = headerFields.reduce(
      ...
      setDynamicFieldsForHeader(initialHeaderFields);
    }

    if (templateToBeUsed?.translations?.[0]?.footer) {
      const footerFields = extractDynamicFields(
        templateToBeUsed?.translations?.[0]?.footer
      );
      const initialFooterFields = footerFields.reduce(
        (acc: Record<string, string>, field: string) => {
          acc[field] = "";
          return acc;
        },
      ...
      setDynamicFieldsForFooter(initialFooterFields);
    }
  }, [templateToBeUsed]);
  ...
  // Prepare dynamic_fields data as plain objects (instead of React elements)
  const dynamicFieldsContentForHeader = Object.entries(
    dynamicFieldsForHeader
  ...
  // Prepare dynamic_fields data as plain objects (instead of React elements)
  const dynamicFieldsContentForFooter = Object.entries(
    dynamicFieldsForFooter
  ).reduce((acc, [key, value]) => {
    // @ts-ignore
    acc[key] = "Your Value Here";
    return acc;
                ...
                {JSON.stringify(
                  {
                    ...
                    language_code:
                      "Your Language Code here for example: en or ar",
                    to: [
                      {
                        number: "Your Number here",
                      },
                    ],
                    components: JSON.parse(templateToBeUsed.translations[0].body),
                  },
                  null,
                  2
                )}
